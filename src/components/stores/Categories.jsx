...
import StoreCategories from "../../api/StoreCategories.json";
          ...
          StoreCategories.map((category) => (
             <div className="px-2" key={category.id}>
               <div className="!h-[290px] w-full overflow-hidden border-2 border-gray-200 rounded-2xl hover:scale-101 transition-all">
                ...
                <h3 className="text-sm md:text-lg lg:text-[1.125rem] font-semibold line-clamp-1">{category.productName}</h3>
                <p className="text-[0.875rem]">{category.productNumber}</p>
                </div>
              </div>
             </div>
          ))}
